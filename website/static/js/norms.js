...
const TABLE_NAME = 'A_NAS_P_NORM';
const TABLE_RUS_NAME = 'Нормы потребления';
const DB_NAME = 'IO';
...
export function norms(){
  ...
  let currentTableName = TABLE_NAME;
  let currentRusName = TABLE_RUS_NAME;
  ...
  const reloadTable = () => {
    createTable(TABLE_NAME, TABLE_RUS_NAME, DB_NAME);
  };

  function showTableNotFound(containerContent) {
  ...
  function createButtonsTable(tableScroll, result, tableRow, rusName, tableName, hasData) {
    ...
    btnDelete.addEventListener('click', () => {
      modalDelete.createModal(reloadTable);
    });

    btnCopy.addEventListener('click', () => {
      modalCopy.createModal(reloadTable);
    });

    btnEdit.addEventListener('click', () => {
      modalEdit.createModal(reloadTable);
    });

    btnInsert.addEventListener('click', () => {
      modalInser.createModal(reloadTable);
    });
  }
  ...
  function createTable(engName, rusName, dbName) {
    ...
    currentTableData = [];
    currentColumnsInfo = [];
    currentTableName = engName || TABLE_NAME;
    currentRusName = rusName || TABLE_RUS_NAME;
    ...
    let data = { name: currentTableName };

    postJSON(data, currentDbName).then(result => {
      console.log(result)
      loader.close();
      if (result === undefined || result === null) {
        showTableNotFound(containerContent);
      } else {
        generateTable(result, currentRusName, currentTableName);
      }
    }).catch(error => {
  ...
  function checkVoidTable(result, tableName, rusName) {
    const containerContent = document.querySelector('.container_content');
    ...
    document.querySelector('.sql-img').addEventListener('click', () => {
  ...
  function generateTable(result, rusName, tableName) {
    if (result.total_rows_count === 0) {
      checkVoidTable(result, tableName, rusName);
    } else {
      createTableContent(result, result.columns_info, tableName, rusName);
    }
  }
